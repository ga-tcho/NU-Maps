...
class CAS_Client
{
    ...
    */
	...
	//XXX ★Client.phpのgetter
	private $_syozokuList = array();
	public function setSyozokuList($syozokuList){
		$this->_syozokuList = $syozokuList;
	}
	public function getSyozokuList(){
		if ( empty($this->_user) ) { // if no user is set, there shouldn't be any attributes also...
			phpCAS::error('this method should be used only after '.__CLASS__.'::forceAuthentication() or '.__CLASS__.'::isAuthenticated()');
		}
		return $this->_syozokuList;
	}
	public function hasSyozokuList(){
		return !empty($this->_syozokuList);
	}

	private $_roleIdList = array();
	public function setRoleIdList($roleIdList){
		$this->_roleIdList = $roleIdList;
	}
	...
	public function hasRoleIdList(){
		return !empty($this->_roleIdList);
	}



	private $_roleHoldIdList = array();
	public function setRoleHoldIdList($roleHoldIdList){
		$this->_roleHoldIdList = $roleHoldIdList;
	}
	public function getRoleHoldIdList() {
		if ( empty($this->_user) ) { // if no user is set, there shouldn't be any attributes also...
			phpCAS::error('this method should be used only after '.__CLASS__.'::forceAuthentication() or '.__CLASS__.'::isAuthenticated()');
		}
		return $this->_roleHoldIdList;
	}
	public function hasRoleHoldIdList(){
		return !empty($this->_roleHoldIdList);
	}


	private $_ijyoUserList = array();
	public function setIjyoUserList($ijyoUserList){
		$this->_ijyoUserList = $ijyoUserList;
	}
	public function getijyoUserList(){
		if ( empty($this->_user) ) { // if no user is set, there shouldn't be any attributes also...
			phpCAS::error('this method should be used only after '.__CLASS__.'::forceAuthentication() or '.__CLASS__.'::isAuthenticated()');
		}
		return $this->_ijyoUserList;
	}
	public function hasIjyoUserList(){
		return !empty($this->_ijyoUserList);
	}
        ...
        if ( $this->_wasPreviouslyAuthenticated() ) {
            ...
            if ($this->hasTicket()) {
                ...
                case CAS_VERSION_3_0:
					...
					//XXX ★ セッションに追加する。
					if($this->hasSyozokuList()){
						$_SESSION['phpCAS']['syozokuList'] = $this->getSyozokuList();
					}
					if($this->hasRoleIdList()){
						$_SESSION['phpCAS']['roleIdList'] = $this->getRoleIdList();
					}
					if($this->hasRoleHoldIdList()){
						$_SESSION['phpCAS']['roleHoldIdList'] = $this->getRoleHoldIdList();
					}
    ...
    private function _wasPreviouslyAuthenticated()
    {
        ...
        if ( $this->isProxy() ) {
            ...
            if ( $this->isSessionAuthenticated() ) {
				...
				//XXX ★ セッションから取得し、再設定する。
				if(isset($_SESSION['phpCAS']['syozokuList'])){
					$this->setSyozokuList($_SESSION['phpCAS']['syozokuList']);
				}
				if(isset($_SESSION['phpCAS']['roleIdList'])){
					$this->setRoleIdList($_SESSION['phpCAS']['roleIdList']);
				}
				if(isset($_SESSION['phpCAS']['roleHoldIdList'])){
					$this->setRoleHoldIdList($_SESSION['phpCAS']['roleHoldIdList']);
				}
    ...
    */
    ...
    public function validateCAS20(&$validate_url,&$text_response,&$tree_response)
    {
        ...
        if ( !($dom->loadXML($text_response))) {
            ...
            if ( $success_elements->item(0)->getElementsByTagName("user")->length == 0) {
				...
				//XXX ★ プロトコルの読み込み呼び出し元（チケット検証）
				$this->readSyozokuList($success_elements);
				$this->readRoleIdList($success_elements);
				$this->readRoleHoldIdList($success_elements);
				$this->readIjyoUserList($success_elements);
        ...
        if ( $success_elements->item(0)->getElementsByTagName("attributes")->length != 0) {
            ...
            if ($attr_nodes->item(0)->hasChildNodes()) {
                ...
                foreach ($attr_nodes->item(0)->childNodes as $attr_child) {
					...
					//XXX ★所属グループの取得
					if($attr_child->localName === "syozoku_group"){
						continue;
					}
					...
					//XXX ★セミコロンを元に戻す
                    $this->_addAttributeToArray(
                        $extra_attributes, str_replace("__", ";", $attr_child->localName),
                        $attr_child->nodeValue
                    );
                }
            }
	...
	//XXX ★ プロトコルの読み込み
	private function readSyozokuList($success_elements){

		phpCAS::traceBegin();
		$resultArray = array();

		$parent = $success_elements->item(0)->getElementsByTagName("syozoku_group");
		if($parent->length == 0){
			return TRUE;
		}
		$child = $parent->item(0)->getElementsByTagName("syozoku");
		if($child->length == 0){
			return TRUE;
		}
		...
		foreach($child as $grandChild){
			...
			$tempArray = array();
			foreach($grandChild->childNodes as $element){
				$this->addSyozokuToArray($tempArray, $element->localName, $element->nodeValue);
			}
			array_push($resultArray, $tempArray);
		}

		$this->setSyozokuList($resultArray);
		phpCAS::traceEnd();
		return TRUE;
	}

	private function addSyozokuToArray (array &$syozokuArray, $name, $value) {
		if (isset($syozokuArray[$name])) {
			if (!is_array($syozokuArray[$name])) {
				$existingValue = $syozokuArray[$name];
				$syozokuArray[$name] = array($existingValue);
			}

			$syozokuArray[$name][] = trim($value);
		} else {
			$syozokuArray[$name] = trim($value);
		}
	}

	private function readRoleIdList($success_elements){

		phpCAS::traceBegin();
		$resultArray = array();

		$parent = $success_elements->item(0)->getElementsByTagName("roles");
		$child = $parent->item(0)->getElementsByTagName("role");
		if($child->length == 0){
			return TRUE;
		}
		...
		foreach($child as $grandChild){
			...
			$tempArray = array();
			foreach($grandChild->childNodes as $element){
				$this->addRoleToArray($tempArray, $element->localName, $element->nodeValue);
			}
			array_push($resultArray, $tempArray);
		}

		$this->setRoleIdList($resultArray);
		phpCAS::traceEnd();
		return TRUE;
	}

	private function addRoleToArray (array &$roleArray, $name, $value) {
		if (isset($roleArray[$name])) {
			if (!is_array($roleArray[$name])) {
				$existingValue = $roleArray[$name];
				$roleArray[$name] = array($existingValue);
			}
			$roleArray[$name][] = trim($value);
		} else {
			$roleArray[$name] = trim($value);
		}
	}

	private function readRoleHoldIdList($success_elements){

		phpCAS::traceBegin();
		$resultArray = array();

		$parent = $success_elements->item(0)->getElementsByTagName("roleHolders");
		$child = $parent->item(0)->getElementsByTagName("roleHolder");
		if($child->length == 0){
			return TRUE;
		}
		...
		foreach($child as $grandChild){
			...
			$tempArray = array();
			foreach($grandChild->childNodes as $element){
				$this->addRoleHoldToArray($tempArray, $element->localName, $element->nodeValue);
			}
			array_push($resultArray, $tempArray);
		}

		$this->setRoleHoldIdList($resultArray);
		phpCAS::traceEnd();
		return TRUE;
	}

	private function addRoleHoldToArray (array &$roleHoldArray, $name, $value) {
		if (isset($roleHoldArray[$name])) {
			if (!is_array($roleHoldArray[$name])) {
				$existingValue = $roleHoldArray[$name];
				$roleHoldArray[$name] = array($existingValue);
			}
			$roleHoldArray[$name][] = trim($value);
		} else {
			$roleHoldArray[$name] = trim($value);
		}
	}
	...
	// 委譲元ユーザの情報を取得するメソッド
	private function readIjyoUserList($success_elements){

		phpCAS::traceBegin();
		$resultArray = array();

		$parent = $success_elements->item(0)->getElementsByTagName("delegationOfAuthorityGroup");
		if($parent->length == 0){
			$this->setIjyoUserList($resultArray);
			return TRUE;
		}
		$child = $parent->item(0)->getElementsByTagName("delegationOfAuthority");
		if($child->length == 0){
			$this->setIjyoUserList($resultArray);
			return TRUE;
		}
		...
		foreach($child as $grandChild){
			...
			// 委譲元ユーザ1人の処理
			$resultUser = new NuCasUser;

			// user属性の取得
			$resultUser->user = trim($grandChild->getElementsByTagName("user")->item(0)->nodeValue);

			// attributesの取得
			if ( $grandChild->getElementsByTagName("attributes")->length != 0) {
				$attr_nodes = $grandChild->getElementsByTagName("attributes");
				if($attr_nodes->item(0)->hasChildNodes()){
					// Nested Attributes
					foreach ($attr_nodes->item(0)->childNodes as $attr_child) {
						...
						$this->addAttributeToArray($resultUser->attributes, str_replace("__", ";", $attr_child->localName), $attr_child->nodeValue);
					}
				}
			}

			// syozoku_groupの取得
			$syozokuParent = $grandChild->getElementsByTagName("syozoku_group");
			if($syozokuParent->length != 0){
				$syozokuChild = $syozokuParent->item(0)->getElementsByTagName("syozoku");
				if($syozokuChild->length != 0){
					foreach($syozokuChild as $syozokuGrandChild){

						$tempSyozokuArray = array();
						foreach($syozokuGrandChild->childNodes as $syozokuElement){
							$this->addSyozokuToArray($tempSyozokuArray, $syozokuElement->localName, $syozokuElement->nodeValue);
						}
						array_push($resultUser->syozokuList, $tempSyozokuArray);
					}
				}
			}
			...
			// rolesの取得
			$roleParent = $grandChild->getElementsByTagName("roles");
			if($roleParent->length != 0){
				$roleChild = $roleParent->item(0)->getElementsByTagName("role");
				if($roleChild->length != 0){
					foreach($roleChild as $roleGrandChild){
						$tempRoleArray = array();
						foreach($roleGrandChild->childNodes as $roleElement){
							//$this->addRoleToArray($tempArray, $element->localName, $element->nodeValue);
							$this->addRoleToArray($tempRoleArray, $roleElement->localName, $roleElement->nodeValue, $roleElement);
						}
						array_push($resultUser->roleIdList, $tempRoleArray);
					}
				}
			}
			...
			// roleholdersの取得
			$roleHolderParent = $grandChild->getElementsByTagName("roleHolders");
			if($roleHolderParent->length != 0){
				$roleHolderChild = $roleHolderParent->item(0)->getElementsByTagName("roleHolder");
				if($roleHolderChild->length != 0){
					foreach($roleHolderChild as $roleHolderGrandChild){

						$tempRoleHolderArray = array();
						foreach($roleHolderGrandChild->childNodes as $roleHolderElement){
							$this->addRoleHoldToArray($tempRoleHolderArray, $roleHolderElement->localName, $roleHolderElement->nodeValue);
						}
						array_push($resultUser->roleHoldIdList, $tempRoleHolderArray);
					}
				}
			}

			// 結果を入れる
			array_push($resultArray, $resultUser);
...
class NuCasUser
{
	...
	public $syozokuList;
	public $roleIdList;
	public $roleHoldIdList;

    public function __construct()
    {
        $this->user = '';
        $this->attributes = array();
        $this->syozokuList = array();
        $this->roleIdList = array();
        $this->roleHoldIdList = array();
    }
