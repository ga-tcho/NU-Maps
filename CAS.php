...
class phpCAS
{
    ...
    */
	...
	//XXX ★CAS.phpのgetter
	public static function getSyozokuList() {
        phpCAS::_validateClientExists();

        try {
            return self::$_PHPCAS_CLIENT->getSyozokuList();
        } catch (Exception $e) {
            phpCAS :: error(get_class($e) . ': ' . $e->getMessage());
        }
	}
	...
	public static function getRoleIdList() {
        ...
        try {
            return self::$_PHPCAS_CLIENT->getRoleIdList();
        } catch (Exception $e) {
            phpCAS :: error(get_class($e) . ': ' . $e->getMessage());
        }
	}

	public static function getRoleHoldIdList() {
        phpCAS::_validateClientExists();

        try {
            return self::$_PHPCAS_CLIENT->getRoleHoldIdList();
        } catch (Exception $e) {
            phpCAS :: error(get_class($e) . ': ' . $e->getMessage());
        }
	}
	...
	// 委譲元ユーザの取得
	public static function getIjyoUserList() {
        phpCAS::_validateClientExists();

        try {
            return self::$_PHPCAS_CLIENT->getIjyoUserList();
        } catch (Exception $e) {
            phpCAS :: error(get_class($e) . ': ' . $e->getMessage());
        }
	}

	// ログインユーザが認証成功しているかの検証（認証成功はtrue、認証失敗はfalse）
	public static function isAuthenticationSuccess() {
        phpCAS::_validateClientExists();

		$authenticationSuccess = true;

		// ログインユーザのロールとロールホルダーの個数が両方0なら、認証失敗。
		$roleList = self::$_PHPCAS_CLIENT->getRoleIdList();
		$roleHoldList = self::$_PHPCAS_CLIENT->getRoleHoldIdList();
		if ( count($roleList) == 0 && count($roleHoldList) == 0 ){
	...
	// ログインユーザの在籍状態を取得する（在籍はT、非在籍はF）
	public static function getEnrollment() {
        phpCAS::_validateClientExists();

		// 所属リストを取得する
		$syozokuList = self::$_PHPCAS_CLIENT->getSyozokuList();
		// 所属情報が0件なら在籍と判断する（権限委譲されている場合のみなので、本来はあり得ない）
		if ( count($syozokuList) == 0 ){
			return "T";
		}

		// 所属リストからenrollmentを見て、1個でも在籍があればTにする
		...
		// 全てのenrollmentがFならば、非在籍とする
		return "F";
	}

	// ロールIDからログインユーザの在籍状態を取得する（在籍はT、非在籍はF）
	public static function getEnrollmentByRole($roleId) {
        phpCAS::_validateClientExists();
		...
		// ロールリストを取得する
		$roleList = self::$_PHPCAS_CLIENT->getRoleIdList();
		// ロールリストから、ロールIDに該当する所属リストを取得する
	    foreach ($roleList as $tmp){
			if ( $tmp["id"] == $roleId ) {
				$syozokuIdList = $tmp["syozoku_id_group"];
			}
		}

		// 所属リストから、所属IDが同じかつenrollment=Tならば、在籍に設定する
		if ( is_array($syozokuIdList) ) {
			foreach ($syozokuIdList as $syozokuId) {
				foreach ($syozokuList as $tmp){
			    	if ( $tmp["enrollment"] == "T" && $tmp["syozoku_id"] == $syozokuId) {
						return "T";
			    	}
			    }
			}
	...
	// ロールホルダーIDからログインユーザの在籍状態を取得する（在籍はT、非在籍はF）
	public static function getEnrollmentByRoleHolder($roleHolderId) {
        phpCAS::_validateClientExists();
		...
		// ロールホルダーリストを取得する
		$roleHolderList = self::$_PHPCAS_CLIENT->getRoleHoldIdList();
		// ロールホルダーリストから、ロールホルダーIDに該当する所属リストを取得する
	    foreach ($roleHolderList as $tmp){
			if ( $tmp["id"] == $roleHolderId ) {
				$syozokuId = $tmp["syozoku_id"];
			}
		}
